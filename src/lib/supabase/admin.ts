 ...
 * INSERT into `orders` / `order_items` from /api/checkout and UPDATE
 * `orders.status` from /api/stripe-webhook regardless of the caller's
 * auth state (which may be anonymous for guest checkout).
 *
  ...
  if (!serviceKey) {
    throw new Error(
      "Missing SUPABASE_SERVICE_ROLE_KEY — this server-only key is " +
        "required for admin operations like order creation and webhook handling. " +
